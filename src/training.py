...
for epoch in range(start_epoch, epochs):
    ...
    # Derive a descriptive loss label from configured losses
    configured_losses = list(config["training"].get("loss", {}).keys())
    loss_label = "+".join(configured_losses) if len(configured_losses) > 1 else (configured_losses[0] if configured_losses else "loss")
    writer.add_scalars(
        f"Loss/train vs validation/{loss_label}",
